...
def save_visualization(dataset, img_meta, result, output_dir, threshold=0.8, fmt='.jpg'):
...
def do_evaluation(model, data_loader, device, types, output_dir, iteration=None, viz=False):
    ...
    if feature_extraction == True:
        ...
        def feature_hook(module, input_, output):
            nonlocal feature_out
            feature_out = output

        def label_hook(module, input_, output):
            nonlocal label_out
            label_out = output
        ...
        # model.box_head.box_predictor.classifier[3].register_forward_hook(get_activation('box_head.box_predictor.classifier[3]'))
        model.box_head.box_predictor.classifier[3].register_forward_hook(feature_hook)
        model.box_head.box_predictor.cls_score.register_forward_hook(label_hook)
    ...
    for images, img_metas, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        # Ground Truth // Comment if not using gt
        ...
        if feature_extraction == True:
            # print()
            # Feature extraction
            # print(model.box_head.box_predictor.classifier[3].weight.data)
            # print(feature_out.tolist())

            # reshaping the array from 3D matrice to 2D matrice.
            features_arr = feature_out
            features_arr = features_arr.cpu().numpy()
            # import matplotlib.pyplot as plt

            # act = activation['box_head.box_predictor.classifier[3]'].cpu().squeeze().numpy()

            # act = act[..., np.newaxis]
            
            import torch.nn.functional as F

            label = F.softmax(label_out, -1)
            label_final = torch.argmax(label, dim=1)
            label = label_final.cpu().numpy()
            ...
            act = np.column_stack((features_arr, label))
        
            print(act)
            print(act.shape)
            
            if flag == False:
                dset = g.create_dataset('baseline', shape=(300,4097), maxshape=(None,None), chunks=True, compression='gzip', compression_opts=9)
                flag = True
            else:
                with h5py.File('mytestfile.hdf5', 'a', libver='latest') as hf:
                    hf["baseline"].resize((hf["baseline"].shape[0] + act.shape[0]), axis = 0)
                    hf["baseline"][-(act.shape[0]):] = act
            ...
            # print("==========")
            # print(len(feature_out_arr[250]))
            # feature_out_arr_reshaped = feature_out_arr.reshape(feature_out_arr.shape[0], -1)
            # feature_list.append(act)
